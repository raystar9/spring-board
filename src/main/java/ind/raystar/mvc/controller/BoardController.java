...
public class BoardController {
	...
	public String boardList(Model model, @RequestParam(name="page", defaultValue="1") String pageParam,
			@RequestParam(required=false) String query, @RequestParam(name="searchby", required=false) String searchBy ,MySession mySession) {
	...
	/*
	 * RequestMapping의 value값과 return값이 일치한다면
	 * 반환형을 void로 해도 같은 결과를 얻을 수 있다.
	 */
	@RequestMapping(value="board/post")
	public String postPost(Model model, @RequestParam("postno") int postNo, MySession mySession) {
		logger.debug(mySession.getId());
		model.addAttribute("post", boardService.getPostDetail(postNo));
		return "board/post";
	}
	
	@RequestMapping(value="board/post", method=RequestMethod.PUT)
	public String putPost(Model model, PostDTO post) {
		boardService.updatePost(post);
		model.addAttribute("post", post);
		return "redirect:/board/post?postno="+post.getPostNo();
	}
	
	@RequestMapping(value="board/post/download", method=RequestMethod.GET)
	public void downloadFile(HttpServletRequest request, HttpServletResponse response, @RequestParam("postno") int postNo) throws Exception{
	...
	public String boardNewInsert(@RequestParam String title, @RequestParam String content, @RequestParam MultipartFile multipartFile,
			MultipartHttpServletRequest request, MySession mySession) throws Exception {
		String rootPath = request.getSession().getServletContext().getRealPath("/");
	...
	@RequestMapping(value="board/post/modify", method=RequestMethod.POST)
	public String boardModify(Model model, @RequestParam("postno") int postNo) {
